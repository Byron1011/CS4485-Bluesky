...
const colorForType = (t) => {
  ...
  if (key === 'tornado')      return cssVar('--marker-tornado')      || '#ff3b30';
  if (key === 'earthquake')   return cssVar('--marker-earthquake')   || '#ff9500';
  if (key === 'flood')        return cssVar('--marker-flood')        || '#34c759';
  if (key === 'thunderstorm') return cssVar('--marker-thunderstorm') || '#5856d6';
  return cssVar('--marker-default') || '#3895ff';
};
...
export default function MapView({
  ...
  //heat dynamics
  //cell size in degrees for different zooms
  ...
  const buildHeatPoints = (z) => {
    const cell = gridSizeForZoom(z);           // degrees lat/lng per cell
    ...
    // Convert post count -> density (posts per km^2)
    // Area = (cell_deg_lat * km/deg) * (cell_deg_lng * km/deg * cos(lat))
    ...
    // normaliza by max density at current zoom
  ...
  //updating heat layer
  const updateHeat = () => {
    if (!mapRef.current) return; // Rebuild layer only if size/blur/opacity changed
  ...
  const rebuildMarkers = () => { 
    ...
    postsRef.current.forEach((p) => {
      ...
      const cm = L.circleMarker([lat, lng], {
        ...
        .on('click', (e) => {
          L.DomEvent.stopPropagation(e); // map click doesnt clear selection
  ...
  useEffect(() => {
    if (!mapRef.current && mapEl.current) {
      ...
      //canvas renderer draws into markers pane
      markersRendererRef.current = L.canvas({ padding: 0.5, pane: 'markers' });

      // update both heat + markers
      const onZoom = () => {
        updateHeat();
        rebuildMarkers();
      };
  ...
  // Recompute overlays when posts change
  useEffect(() => {
    if (!mapRef.current) return;
    postsRef.current = posts;

    console.log("MapView received posts:", posts.length, posts.slice(0, 2));
  ...
  useEffect(() => {
    ...
    // deselect restores styles & closes popup
